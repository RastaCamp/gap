...
CREATE TABLE IF NOT EXISTS readings (
  id             TEXT PRIMARY KEY,
  source         TEXT NOT NULL CHECK(source IN ('eia','noaa_swpc')),
  source_id      TEXT NOT NULL,
  title          TEXT NOT NULL,
  summary        TEXT NOT NULL DEFAULT '',
  region         TEXT,
  reading_type   TEXT NOT NULL DEFAULT 'demand',
  value          REAL,
  unit           TEXT,
  observed_at    TEXT,
  source_url     TEXT,
...
CREATE VIRTUAL TABLE IF NOT EXISTS readings_fts USING fts5(
  title, summary, region, reading_type, content='readings', content_rowid='rowid'
);
CREATE TRIGGER IF NOT EXISTS readings_fts_insert AFTER INSERT ON readings BEGIN
  INSERT INTO readings_fts(rowid, title, summary, region, reading_type) VALUES (new.rowid, new.title, new.summary, new.region, new.reading_type);
END;
CREATE TRIGGER IF NOT EXISTS readings_fts_update AFTER UPDATE ON readings BEGIN
  INSERT INTO readings_fts(readings_fts, rowid, title, summary, region, reading_type) VALUES ('delete', old.rowid, old.title, old.summary, old.region, old.reading_type);
  INSERT INTO readings_fts(rowid, title, summary, region, reading_type) VALUES (new.rowid, new.title, new.summary, new.region, new.reading_type);
END;
