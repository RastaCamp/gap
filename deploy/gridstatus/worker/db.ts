...
export async function queryReadings(
  env: WorkerEnv,
  params: { q?: string; source?: string; reading_type?: string; region?: string; page?: number; per_page?: number }
): Promise<{ rows: Reading[]; total: number; source_counts: Record<string, number> }> {
  ...
  if (params.source) {
    conditions.push("readings.source = ?");
    args.push(params.source);
  }
  if (params.reading_type) {
    conditions.push("readings.reading_type = ?");
    args.push(params.reading_type);
  }
  if (params.region) {
    conditions.push("readings.region = ?");
    args.push(params.region);
  }

  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";

  const totalRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM readings ${where}`)
    .bind(...args)
    .first<{ count: number }>();
  ...
  const sourceCounts = await env.DB.prepare(`SELECT source, COUNT(*) as count FROM readings ${where} GROUP BY source`)
    .bind(...args)
...
export async function getStats(env: WorkerEnv) {
  const total =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM readings").first<{ count: number }>())?.count ?? 0;
  const bySource = await env.DB.prepare("SELECT source, COUNT(*) as count FROM readings GROUP BY source").all<{
    source: string;
    count: number;
  }>();
  const byType = await env.DB.prepare("SELECT reading_type, COUNT(*) as count FROM readings GROUP BY reading_type").all<{
    reading_type: string;
    count: number;
  }>();
  ...
  const pending =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM sync_jobs WHERE status IN ('pending','running')").first<{
      count: number;
    }>())?.count ?? 0;
  ...
  return {
    ...
    by_reading_type: Object.fromEntries((byType.results ?? []).map((r) => [r.reading_type, r.count])),
