...
CREATE TABLE IF NOT EXISTS reports (
  id           TEXT PRIMARY KEY,
  source       TEXT NOT NULL CHECK(source IN ('cdc','aphis','promed','birdcast')),
...
CREATE VIRTUAL TABLE IF NOT EXISTS reports_fts USING fts5(title, summary, report_type, location, content='reports', content_rowid='rowid');
CREATE TRIGGER IF NOT EXISTS reports_fts_insert AFTER INSERT ON reports BEGIN
  INSERT INTO reports_fts(rowid, title, summary, report_type, location) VALUES (new.rowid, new.title, new.summary, new.report_type, new.location);
END;
CREATE TRIGGER IF NOT EXISTS reports_fts_update AFTER UPDATE ON reports BEGIN
  INSERT INTO reports_fts(reports_fts, rowid, title, summary, report_type, location) VALUES ('delete', old.rowid, old.title, old.summary, old.report_type, old.location);
  INSERT INTO reports_fts(rowid, title, summary, report_type, location) VALUES (new.rowid, new.title, new.summary, new.report_type, new.location);
END;
