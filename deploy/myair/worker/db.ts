...
export async function queryReadings(
  env: WorkerEnv,
  params: { q?: string; source?: string; category?: string; page?: number; per_page?: number }
  ...
  if (params.category) {
    conditions.push("readings.category = ?");
...
export async function getStats(env: WorkerEnv) {
  ...
  const byCategory = await env.DB.prepare("SELECT category, COUNT(*) as count FROM readings GROUP BY category").all<{
    category: string;
    count: number;
  }>();
