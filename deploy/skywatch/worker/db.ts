...
export async function queryData(
  env: WorkerEnv,
  params: { q?: string; source?: string; observation_type?: string; page?: number; per_page?: number }
  ...
  if (params.q) {
    conditions.push(`observations.rowid IN (SELECT rowid FROM observations_fts WHERE observations_fts MATCH ?)`);
    args.push(params.q.replace(/['"]/g, "") + "*");
  }
  if (params.source) {
    conditions.push("observations.source = ?");
    args.push(params.source);
  }
  if (params.observation_type) {
    conditions.push("observations.observation_type = ?");
    args.push(params.observation_type);
  }

  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";

  const totalRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM observations ${where}`)
    .bind(...args)
    .first<{ count: number }>();
  const total = totalRow?.count ?? 0;

  const rows = await env.DB.prepare(
    `SELECT * FROM observations ${where} ORDER BY observed_at DESC, created_at DESC LIMIT ? OFFSET ?`
  )
    .bind(...args, per_page, offset)
    .all<DataRow>();

  const sourceCounts = await env.DB.prepare(
    `SELECT source, COUNT(*) as count FROM observations ${where} GROUP BY source`
  )
    .bind(...args)
...
export async function getStats(env: WorkerEnv) {
  const total =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM observations").first<{ count: number }>())?.count ?? 0;
  const bySource = await env.DB.prepare("SELECT source, COUNT(*) as count FROM observations GROUP BY source").all<{
    source: string;
    count: number;
  }>();
