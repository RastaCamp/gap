...
export async function queryData(
  env: WorkerEnv,
  params: { q?: string; source?: string; alert_type?: string; page?: number; per_page?: number }
  ...
  if (params.q) {
    conditions.push(`alerts.rowid IN (SELECT rowid FROM alerts_fts WHERE alerts_fts MATCH ?)`);
    args.push(params.q.replace(/['"]/g, "") + "*");
  }
  if (params.source) {
    conditions.push("alerts.source = ?");
    args.push(params.source);
  }
  if (params.alert_type) {
    conditions.push("alerts.alert_type = ?");
    args.push(params.alert_type);
  }

  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";

  const totalRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM alerts ${where}`)
    .bind(...args)
    .first<{ count: number }>();
  const total = totalRow?.count ?? 0;

  const rows = await env.DB.prepare(
    `SELECT * FROM alerts ${where} ORDER BY effective_at DESC, created_at DESC LIMIT ? OFFSET ?`
  )
    .bind(...args, per_page, offset)
    .all<DataRow>();

  const sourceCounts = await env.DB.prepare(
    `SELECT source, COUNT(*) as count FROM alerts ${where} GROUP BY source`
  )
    .bind(...args)
...
export async function getStats(env: WorkerEnv) {
  const total =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM alerts").first<{ count: number }>())?.count ?? 0;
  const bySource = await env.DB.prepare("SELECT source, COUNT(*) as count FROM alerts GROUP BY source").all<{
    source: string;
    count: number;
  }>();
