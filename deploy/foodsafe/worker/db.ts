...
export async function queryData(
  env: WorkerEnv,
  params: { q?: string; source?: string; recall_type?: string; page?: number; per_page?: number }
  ...
  if (params.q) {
    conditions.push(`recalls.rowid IN (SELECT rowid FROM recalls_fts WHERE recalls_fts MATCH ?)`);
  ...
  if (params.recall_type) {
    conditions.push("recalls.recall_type = ?");
    args.push(params.recall_type);
  }

  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";

  const totalRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM recalls ${where}`)
    .bind(...args)
    .first<{ count: number }>();
  const total = totalRow?.count ?? 0;

  const rows = await env.DB.prepare(
    `SELECT * FROM recalls ${where} ORDER BY reported_at DESC, created_at DESC LIMIT ? OFFSET ?`
  )
...
export async function getStats(env: WorkerEnv) {
  const total =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM recalls").first<{ count: number }>())?.count ?? 0;
  const bySource = await env.DB.prepare("SELECT source, COUNT(*) as count FROM recalls GROUP BY source").all<{
    source: string;
    count: number;
  }>();
