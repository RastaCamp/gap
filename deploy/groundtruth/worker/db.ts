...
export async function queryData(
  env: WorkerEnv,
  params: { q?: string; source?: string; event_type?: string; page?: number; per_page?: number }
  ...
  if (params.q) {
    conditions.push(`events.rowid IN (SELECT rowid FROM events_fts WHERE events_fts MATCH ?)`);
    args.push(params.q.replace(/['"]/g, "") + "*");
  }
  if (params.source) {
    conditions.push("events.source = ?");
  ...
  const totalRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM events ${where}`)
    .bind(...args)
    .first<{ count: number }>();
  const total = totalRow?.count ?? 0;

  const rows = await env.DB.prepare(
    `SELECT * FROM events ${where} ORDER BY occurred_at DESC, created_at DESC LIMIT ? OFFSET ?`
  )
    .bind(...args, per_page, offset)
    .all<DataRow>();

  const sourceCounts = await env.DB.prepare(
    `SELECT source, COUNT(*) as count FROM events ${where} GROUP BY source`
  )
    .bind(...args)
...
export async function getStats(env: WorkerEnv) {
  const total =
    (await env.DB.prepare("SELECT COUNT(*) as count FROM events").first<{ count: number }>())?.count ?? 0;
  const bySource = await env.DB.prepare("SELECT source, COUNT(*) as count FROM events GROUP BY source").all<{
    source: string;
    count: number;
  }>();
