...
export async function handleLogin(req: Request): Promise<Response> {
  ...
  const email = (body.email ?? "").trim(); const role = (body.role ?? "user") as string;
  if (!email) return json({ error: "email required" }, 400);
  let user = getUserByEmail(email); if (!user) user = createUser({ email, name: email.split("@")[0], region: "", role });
  const token = crypto.randomUUID(); tokenToUserId.set(token, user.id);
...
export function handleMe(req: Request): Response {
  ...
  const userId = tokenToUserId.get(token); if (!userId) return json({ error: "Unauthorized" }, 401);
  const user = getUserById(userId); if (!user) return json({ error: "User not found" }, 404);
  const usage = getUsageForUser(userId); const period = new Date().toISOString().slice(0, 10); recordUsage(userId, period);
...
export function handleDebugLogin(_req: Request): Response {
  if (process.env.NODE_ENV === "production" || process.env.ENABLE_DEBUG_LOGIN === "false") return json({ error: "Debug login disabled" }, 403);
...
export function isAdmin(req: Request): boolean {
  const auth = req.headers.get("Authorization") ?? ""; const token = auth.replace(/^Bearer\s+/i, "").trim();
  if (token === DEBUG_TOKEN) return true; if (token === ADMIN_TOKEN) return true; const userId = tokenToUserId.get(token); if (!userId) return false;
  const user = getUserById(userId); return user?.role === "admin" || user?.role === "debug";
}
