...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/" || path === "/api") return json({
      name: "BioSurge API",
      version: "0.1.0",
      description: "Disease outbreaks, CDC, ProMED, BirdCast",
      ...
      query_params: { q: "Search", source: "cdc | aphis | promed | birdcast", report_type: "Filter", page: "Page", per_page: "Per page" },
    });
...
console.log(`BioSurge API http://localhost:${PORT}  GET /api/reports  POST /api/admin/sync (auth)`);
