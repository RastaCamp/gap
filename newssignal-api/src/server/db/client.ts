...
import type { Alert, SyncJob, NormalizedRecord } from "../../shared/types";

const DB_PATH = process.env.DB_PATH ?? join(process.cwd(), "data", "newssignal.db");
...
export function upsertAlert(record: NormalizedRecord & { id: string }): "inserted" | "updated" | "unchanged" {
  const db = getDb();
  const existing = db.query<{ content_hash: string }, string>("SELECT content_hash FROM alerts WHERE source = ? AND source_id = ?").get(record.source, record.source_id);
  if (!existing) {
    db.run(`
      INSERT INTO alerts (id, source, source_id, title, summary, alert_type, severity, region, effective_at, expires_at, source_url, raw_json, content_hash, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `, record.id, record.source, record.source_id, record.title, record.summary, record.alert_type ?? null, record.severity ?? null, record.region ?? null, record.effective_at ?? null, record.expires_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash);
    return "inserted";
  }
  if (existing.content_hash === record.content_hash) return "unchanged";
  db.run(`
    UPDATE alerts SET title = ?, summary = ?, alert_type = ?, severity = ?, region = ?, effective_at = ?, expires_at = ?, source_url = ?, raw_json = ?, content_hash = ?, updated_at = datetime('now')
    WHERE source = ? AND source_id = ?
  `, record.title, record.summary, record.alert_type ?? null, record.severity ?? null, record.region ?? null, record.effective_at ?? null, record.expires_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash, record.source, record.source_id);
  return "updated";
}

export function queryAlerts(params: { q?: string; source?: string; alert_type?: string; page?: number; per_page?: number }): { rows: Alert[]; total: number; source_counts: Record<string, number> } {
  const db = getDb();
  ...
  if (params.q) { conditions.push("alerts.rowid IN (SELECT rowid FROM alerts_fts WHERE alerts_fts MATCH ?)"); args.push((params.q.replace(/['"]/g, "")) + "*"); }
  if (params.source) { conditions.push("alerts.source = ?"); args.push(params.source); }
  if (params.alert_type) { conditions.push("alerts.alert_type = ?"); args.push(params.alert_type); }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  const total = db.query<{ count: number }, (string | number)[]>(`SELECT COUNT(*) as count FROM alerts ${where}`).get(...args)?.count ?? 0;
  const rows = db.query<Alert, (string | number)[]>(`SELECT * FROM alerts ${where} ORDER BY effective_at DESC, created_at DESC LIMIT ? OFFSET ?`).all(...args, per_page, offset);
  const sourceCounts = db.query<{ source: string; count: number }, (string | number)[]>(`SELECT source, COUNT(*) as count FROM alerts ${where} GROUP BY source`).all(...args);
...
export function getStats() {
  const db = getDb();
  const total = db.query<{ count: number }, []>("SELECT COUNT(*) as count FROM alerts").get()?.count ?? 0;
  const bySource = db.query<{ source: string; count: number }, []>("SELECT source, COUNT(*) as count FROM alerts GROUP BY source").all();
  const lastJob = db.query<SyncJob, []>("SELECT * FROM sync_jobs WHERE status = 'success' ORDER BY finished_at DESC LIMIT 1").get();
  return { total_alerts: total, by_source: Object.fromEntries(bySource.map(r => [r.source, r.count])), last_successful_sync: lastJob?.finished_at ?? null };
}
