...
import { handleAlerts, handleStats } from "./routes/alerts";
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (method === "GET" && path === "/api/alerts") return handleAlerts(req);
    ...
    if (path === "/" || path === "/api") return json({
      name: "NewsSignal API",
      version: "0.1.0",
      description: "RSS and news aggregation — NOAA Weather Alerts, GDACS",
      endpoints: { "GET /api/alerts": "Query alerts", "GET /api/stats": "Stats", "GET /api/health": "Health", "POST /api/admin/sync": "Sync (auth)", "GET /api/admin/jobs": "Jobs (auth)", "GET /api/admin/jobs/:id": "Job detail (auth)", "GET /api/admin/status": "Status (auth)" },
      query_params: { q: "Search", source: "noaa_alerts | gdacs", alert_type: "Filter", page: "Page", per_page: "Per page" },
    });
...
console.log(`NewsSignal API http://localhost:${PORT}  GET /api/alerts  POST /api/admin/sync (auth)`);
