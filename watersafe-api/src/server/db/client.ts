...
const DB_PATH = process.env.DB_PATH ?? join(process.cwd(), "data", "watersafe.db");
...
export function upsertReport(record: NormalizedRecord & { id: string }): "inserted" | "updated" | "unchanged" {
  ...
  if (!existing) {
    db.run(`
      INSERT INTO reports (id, source, source_id, title, summary, system_name, system_id, location_zip, state, compliance, reported_at, source_url, raw_json, content_hash, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `, record.id, record.source, record.source_id, record.title, record.summary, record.system_name ?? null, record.system_id ?? null, record.location_zip ?? null, record.state ?? null, record.compliance ?? null, record.reported_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash);
    return "inserted";
  }
  if (existing.content_hash === record.content_hash) return "unchanged";
  db.run(`
    UPDATE reports SET title = ?, summary = ?, system_name = ?, system_id = ?, location_zip = ?, state = ?, compliance = ?, reported_at = ?, source_url = ?, raw_json = ?, content_hash = ?, updated_at = datetime('now')
    WHERE source = ? AND source_id = ?
  `, record.title, record.summary, record.system_name ?? null, record.system_id ?? null, record.location_zip ?? null, record.state ?? null, record.compliance ?? null, record.reported_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash, record.source, record.source_id);
  return "updated";
}

export function queryReports(params: { q?: string; source?: string; state?: string; page?: number; per_page?: number }): { rows: Report[]; total: number; source_counts: Record<string, number> } {
  const db = getDb();
