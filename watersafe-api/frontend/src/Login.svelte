<script>
  let email = ""; let password = ""; let error = ""; let token = ""; let user = null; let isAdminLogin = false;
  ...
  async function doLogin(role = "user") {
    error = "";
    const res = await fetch("/api/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: email || "user@example.com", role: isAdminLogin ? "admin" : role }) });
...
<div class="login">
  ...
  <form on:submit|preventDefault={() => doLogin()}>
    ...
    <div class="buttons"><button type="submit">Sign in as User</button><button type="button" on:click={() => doLogin("admin")}>Sign in as Admin</button></div>
  </form>
