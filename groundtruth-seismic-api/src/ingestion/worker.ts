...
import { fetchUsgsEarthquake } from "./sources/usgs_earthquake";
import { fetchIris } from "./sources/iris";
import { fetchVaac } from "./sources/vaac";
import { fetchRsoe } from "./sources/rsoe";
import { fetchTsunami } from "./sources/tsunami";
import { fetchSmithsonianGvp } from "./sources/smithsonian_gvp";
import { runDiff, summarizeUpdate } from "./diff";
import { createSyncJob, updateSyncJob, upsertEvent, insertDiffLog, getDb } from "../server/db/client";

const SOURCES: Record<SourceName, () => Promise<NormalizedRecord[]>> = {
  usgs_earthquake: fetchUsgsEarthquake,
  iris: fetchIris,
  vaac: fetchVaac,
  rsoe: fetchRsoe,
  tsunami: fetchTsunami,
...
export async function runSource(source: SourceName, jobId: string): Promise<{ added: number; updated: number; removed: number; snapshotFile: string }> {
  ...
  if (freshRecords.length === 0 && source !== "usgs_earthquake" && source !== "tsunami" && source !== "smithsonian_gvp") return { added: 0, updated: 0, removed: 0, snapshotFile: "" };
  ...
  const result = db.transaction(() => {
    ...
    for (const record of diff.added) {
      if (upsertEvent({ ...record, id: crypto.randomUUID() }) === "inserted") {
        added++;
    ...
    for (const { previous, current } of diff.updated) {
      if (upsertEvent({ ...current, id: crypto.randomUUID() }) === "updated") {
        updated++;
