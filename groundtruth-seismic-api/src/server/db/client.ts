...
import type { Event, SyncJob, NormalizedRecord } from "../../shared/types";

const DB_PATH = process.env.DB_PATH ?? join(process.cwd(), "data", "groundtruth-seismic.db");
...
export function upsertEvent(record: NormalizedRecord & { id: string }): "inserted" | "updated" | "unchanged" {
  const db = getDb();
  const existing = db.query<{ content_hash: string }, string>("SELECT content_hash FROM events WHERE source = ? AND source_id = ?").get(record.source, record.source_id);
  if (!existing) {
    db.run(`
      INSERT INTO events (id, source, source_id, title, summary, event_type, magnitude, latitude, longitude, depth_km, occurred_at, source_url, raw_json, content_hash, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `, record.id, record.source, record.source_id, record.title, record.summary, record.event_type, record.magnitude ?? null, record.latitude ?? null, record.longitude ?? null, record.depth_km ?? null, record.occurred_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash);
    return "inserted";
  }
  if (existing.content_hash === record.content_hash) return "unchanged";
  db.run(`
    UPDATE events SET title = ?, summary = ?, event_type = ?, magnitude = ?, latitude = ?, longitude = ?, depth_km = ?, occurred_at = ?, source_url = ?, raw_json = ?, content_hash = ?, updated_at = datetime('now')
    WHERE source = ? AND source_id = ?
  `, record.title, record.summary, record.event_type, record.magnitude ?? null, record.latitude ?? null, record.longitude ?? null, record.depth_km ?? null, record.occurred_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash, record.source, record.source_id);
  return "updated";
}

export function queryEvents(params: { q?: string; source?: string; event_type?: string; page?: number; per_page?: number }): { rows: Event[]; total: number; source_counts: Record<string, number> } {
  const db = getDb();
  ...
  if (params.q) { conditions.push("events.rowid IN (SELECT rowid FROM events_fts WHERE events_fts MATCH ?)"); args.push((params.q.replace(/['"]/g, "")) + "*"); }
  if (params.source) { conditions.push("events.source = ?"); args.push(params.source); }
  if (params.event_type) { conditions.push("events.event_type = ?"); args.push(params.event_type); }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  const total = db.query<{ count: number }, (string | number)[]>(`SELECT COUNT(*) as count FROM events ${where}`).get(...args)?.count ?? 0;
  const rows = db.query<Event, (string | number)[]>(`SELECT * FROM events ${where} ORDER BY occurred_at DESC, created_at DESC LIMIT ? OFFSET ?`).all(...args, per_page, offset);
  const sourceCounts = db.query<{ source: string; count: number }, (string | number)[]>(`SELECT source, COUNT(*) as count FROM events ${where} GROUP BY source`).all(...args);
...
export function updateSyncJob(id: string, updates: Partial<SyncJob>): void {
  const db = getDb();
  db.run(`UPDATE sync_jobs SET status = COALESCE(?, status), finished_at = COALESCE(?, finished_at), records_added = COALESCE(?, records_added), records_updated = COALESCE(?, records_updated), records_removed = COALESCE(?, records_removed), error_message = COALESCE(?, error_message), snapshot_files = COALESCE(?, snapshot_files) WHERE id = ?`,
...
export function getStats() {
  const db = getDb();
  const total = db.query<{ count: number }, []>("SELECT COUNT(*) as count FROM events").get()?.count ?? 0;
  const bySource = db.query<{ source: string; count: number }, []>("SELECT source, COUNT(*) as count FROM events GROUP BY source").all();
  const lastJob = db.query<SyncJob, []>("SELECT * FROM sync_jobs WHERE status = 'success' ORDER BY finished_at DESC LIMIT 1").get();
  return { total_events: total, by_source: Object.fromEntries(bySource.map(r => [r.source, r.count])), last_successful_sync: lastJob?.finished_at ?? null };
}
