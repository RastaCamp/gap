...
export async function handleSync(req: Request): Promise<Response> {
  ...
  const valid = ["all", "usgs_earthquake", "iris", "vaac", "rsoe"];
  ...
  (source === "all" ? runAllSources() : runSingleSource(source as SourceName))
    .then(() => runningJobs.delete(source))
    .catch(() => runningJobs.delete(source));
  await Bun.sleep(100);
