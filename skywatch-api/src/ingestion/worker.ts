...
import { fetchNoaaSwpc } from "./sources/noaa_swpc";
import { fetchN2yo } from "./sources/n2yo";
import { fetchMetNo } from "./sources/met_no";
import { runDiff, summarizeUpdate } from "./diff";
import { createSyncJob, updateSyncJob, upsertObservation, insertDiffLog, getDb } from "../server/db/client";

const SOURCES: Record<SourceName, () => Promise<NormalizedRecord[]>> = {
  noaa_swpc: fetchNoaaSwpc,
  n2yo: fetchN2yo,
  met_no: fetchMetNo,
};
...
export async function runSource(source: SourceName, jobId: string): Promise<{ added: number; updated: number; removed: number; snapshotFile: string }> {
  ...
  if (freshRecords.length === 0 && source !== "noaa_swpc") return { added: 0, updated: 0, removed: 0, snapshotFile: "" };

  const diff = runDiff(source, freshRecords);
  const result = getDb().transaction(() => {
    let added = 0, updated = 0;
    for (const record of diff.added) {
      if (upsertObservation({ ...record, id: crypto.randomUUID() }) === "inserted") {
        added++;
    ...
    for (const { previous, current } of diff.updated) {
      if (upsertObservation({ ...current, id: crypto.randomUUID() }) === "updated") {
        updated++;
