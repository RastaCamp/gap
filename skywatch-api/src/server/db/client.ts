...
import type { Observation, SyncJob, NormalizedRecord } from "../../shared/types";

const DB_PATH = process.env.DB_PATH ?? join(process.cwd(), "data", "skywatch.db");
...
export function upsertObservation(record: NormalizedRecord & { id: string }): "inserted" | "updated" | "unchanged" {
  const db = getDb();
  const existing = db.query<{ content_hash: string }, string>("SELECT content_hash FROM observations WHERE source = ? AND source_id = ?").get(record.source, record.source_id);
  if (!existing) {
    db.run(`
      INSERT INTO observations (id, source, source_id, title, summary, observation_type, value, unit, observed_at, source_url, raw_json, content_hash, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `, record.id, record.source, record.source_id, record.title, record.summary, record.observation_type, record.value ?? null, record.unit ?? null, record.observed_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash);
    return "inserted";
  }
  if (existing.content_hash === record.content_hash) return "unchanged";
  db.run(`
    UPDATE observations SET title = ?, summary = ?, observation_type = ?, value = ?, unit = ?, observed_at = ?, source_url = ?, raw_json = ?, content_hash = ?, updated_at = datetime('now')
    WHERE source = ? AND source_id = ?
  `, record.title, record.summary, record.observation_type, record.value ?? null, record.unit ?? null, record.observed_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash, record.source, record.source_id);
  return "updated";
}

export function queryObservations(params: { q?: string; source?: string; observation_type?: string; page?: number; per_page?: number }): { rows: Observation[]; total: number; source_counts: Record<string, number> } {
  const db = getDb();
  ...
  if (params.q) { conditions.push("observations.rowid IN (SELECT rowid FROM observations_fts WHERE observations_fts MATCH ?)"); args.push((params.q.replace(/['"]/g, "")) + "*"); }
  if (params.source) { conditions.push("observations.source = ?"); args.push(params.source); }
  if (params.observation_type) { conditions.push("observations.observation_type = ?"); args.push(params.observation_type); }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  const total = db.query<{ count: number }, (string | number)[]>(`SELECT COUNT(*) as count FROM observations ${where}`).get(...args)?.count ?? 0;
  const rows = db.query<Observation, (string | number)[]>(`SELECT * FROM observations ${where} ORDER BY observed_at DESC, created_at DESC LIMIT ? OFFSET ?`).all(...args, per_page, offset);
  const sourceCounts = db.query<{ source: string; count: number }, (string | number)[]>(`SELECT source, COUNT(*) as count FROM observations ${where} GROUP BY source`).all(...args);
...
export function getStats() {
  const db = getDb();
  const total = db.query<{ count: number }, []>("SELECT COUNT(*) as count FROM observations").get()?.count ?? 0;
  const bySource = db.query<{ source: string; count: number }, []>("SELECT source, COUNT(*) as count FROM observations GROUP BY source").all();
  const lastJob = db.query<SyncJob, []>("SELECT * FROM sync_jobs WHERE status = 'success' ORDER BY finished_at DESC LIMIT 1").get();
  return { total_observations: total, by_source: Object.fromEntries(bySource.map(r => [r.source, r.count])), last_successful_sync: lastJob?.finished_at ?? null };
}
