...
import { handleObservations, handleStats } from "./routes/observations";
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (method === "GET" && path === "/api/observations") return handleObservations(req);
    ...
    if (path === "/" || path === "/api") return json({
      name: "SkyWatch API",
      version: "0.1.0",
      description: "Space weather, solar, satellites — NOAA SWPC, N2YO, MET Norway",
      endpoints: { "GET /api/observations": "Query observations", "GET /api/stats": "Stats", "GET /api/health": "Health", "POST /api/admin/sync": "Sync (auth)", "GET /api/admin/jobs": "Jobs (auth)", "GET /api/admin/jobs/:id": "Job detail (auth)", "GET /api/admin/status": "Status (auth)" },
      query_params: { q: "Search", source: "noaa_swpc | n2yo | met_no", observation_type: "Filter", page: "Page", per_page: "Per page" },
    });
...
console.log(`SkyWatch API http://localhost:${PORT}  GET /api/observations  POST /api/admin/sync (auth)`);
