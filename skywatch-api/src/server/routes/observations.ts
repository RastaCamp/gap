import { queryObservations, getStats } from "../db/client";

function json(data: unknown, status = 200): Response {
  return new Response(JSON.stringify(data), { status, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
}

export async function handleObservations(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const p = url.searchParams;
  const params = {
    q: p.get("q") ?? undefined,
    source: p.get("source") ?? undefined,
    observation_type: p.get("observation_type") ?? undefined,
  ...
  const { rows, total, source_counts } = queryObservations(params);
