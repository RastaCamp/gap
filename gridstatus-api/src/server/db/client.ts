...
export function upsertReading(record: NormalizedRecord & { id: string }): "inserted" | "updated" | "unchanged" {
  const db = getDb();
  const existing = db.query<{ content_hash: string }, string>("SELECT content_hash FROM readings WHERE source = ? AND source_id = ?").get(record.source, record.source_id);
  if (!existing) {
    db.run(`
      INSERT INTO readings (id, source, source_id, title, summary, region, reading_type, value, unit, observed_at, source_url, raw_json, content_hash, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `, record.id, record.source, record.source_id, record.title, record.summary, record.region ?? null, record.reading_type, record.value ?? null, record.unit ?? null, record.observed_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash);
    return "inserted";
  }
  if (existing.content_hash === record.content_hash) return "unchanged";
  db.run(`
    UPDATE readings SET title = ?, summary = ?, region = ?, reading_type = ?, value = ?, unit = ?, observed_at = ?, source_url = ?, raw_json = ?, content_hash = ?, updated_at = datetime('now')
    WHERE source = ? AND source_id = ?
  `, record.title, record.summary, record.region ?? null, record.reading_type, record.value ?? null, record.unit ?? null, record.observed_at ?? null, record.source_url ?? null, record.raw_json, record.content_hash, record.source, record.source_id);
  return "updated";
}

export function queryReadings(params: { q?: string; source?: string; reading_type?: string; region?: string; page?: number; per_page?: number }): { rows: Reading[]; total: number; source_counts: Record<string, number> } {
  const db = getDb();
  ...
  if (params.q) { conditions.push("readings.rowid IN (SELECT rowid FROM readings_fts WHERE readings_fts MATCH ?)"); args.push((params.q.replace(/['"]/g, "")) + "*"); }
  if (params.source) { conditions.push("readings.source = ?"); args.push(params.source); }
  if (params.reading_type) { conditions.push("readings.reading_type = ?"); args.push(params.reading_type); }
  if (params.region) { conditions.push("readings.region = ?"); args.push(params.region); }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  const total = db.query<{ count: number }, (string | number)[]>(`SELECT COUNT(*) as count FROM readings ${where}`).get(...args)?.count ?? 0;
  const rows = db.query<Reading, (string | number)[]>(`SELECT * FROM readings ${where} ORDER BY observed_at DESC, created_at DESC LIMIT ? OFFSET ?`).all(...args, per_page, offset);
  const sourceCounts = db.query<{ source: string; count: number }, (string | number)[]>(`SELECT source, COUNT(*) as count FROM readings ${where} GROUP BY source`).all(...args);
