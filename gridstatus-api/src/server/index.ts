...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/" || path === "/api") return json({
      name: "GridStatus API",
      version: "0.1.0",
      description: "Grid demand, outages, geomagnetic risk — EIA, NOAA SWPC",
      endpoints: { "GET /api/readings": "Query readings", "GET /api/stats": "Stats", "GET /api/health": "Health", "POST /api/admin/sync": "Sync (auth)", "GET /api/admin/jobs": "Jobs (auth)", "GET /api/admin/jobs/:id": "Job detail (auth)", "GET /api/admin/status": "Status (auth)" },
      query_params: { q: "Search", source: "eia | noaa_swpc", reading_type: "demand | geomagnetic", region: "Filter", page: "Page", per_page: "Per page" },
    });
...
console.log(`GridStatus API http://localhost:${PORT}  GET /api/readings  POST /api/admin/sync (auth)`);
