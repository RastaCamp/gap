...
import { createSyncJob, updateSyncJob, upsertReading, insertDiffLog, getDb } from "../server/db/client";
...
export async function runSource(source: SourceName, jobId: string): Promise<{ added: number; updated: number; removed: number; snapshotFile: string }> {
  ...
  const result = getDb().transaction(() => {
    ...
    for (const record of diff.added) {
      if (upsertReading({ ...record, id: crypto.randomUUID() }) === "inserted") {
        added++;
    ...
    for (const { previous, current } of diff.updated) {
      if (upsertReading({ ...current, id: crypto.randomUUID() }) === "updated") {
        updated++;
