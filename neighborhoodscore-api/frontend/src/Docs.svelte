...
<div class="page">
  <h1>API Docs</h1>
  <p class="hero">Local safety and environmental intelligence. One neighborhood risk score from crime, environment, and health data.</p>

  <section>
    <h2>Score by address</h2>
    <p>Get composite neighborhood score and factor sub-scores by address (Walk Score for safety/health):</p>
    <pre class="code">GET /score?address=123+Main+Baltimore&factors=crime,air,water,health</pre>
    <p><strong>Parameters:</strong> <code>address</code> (required), <code>factors</code> (optional: crime, air, water, health). Current backend may expose <code>/api/reports</code>; product shape below is the target API.</p>
  </section>

  <section>
    <h2>Response shape</h2>
    <pre class="code">{`{
  "address": "123 Main, Baltimore",
  "score": 64,
  "factors": { "crime": 48, "air": 32, "water": 55, "health": 21 }
}`}</pre>
    <p>Composite neighborhood intelligence: crime heatmaps, CDC chronic disease, EPA air, water safety, registry data in one address-level score.</p>
  </section>
