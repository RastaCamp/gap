...
import { fetchBaltimore } from "./sources/baltimore";
import { fetchCityprotect } from "./sources/cityprotect";
import { fetchCdcPlaces } from "./sources/cdc_places";
import { fetchAirnow } from "./sources/airnow";
import { fetchNsopw } from "./sources/nsopw";
import { runDiff, summarizeUpdate } from "./diff";
import { createSyncJob, updateSyncJob, upsertReport, insertDiffLog, getDb } from "../server/db/client";

const SOURCES: Record<SourceName, () => Promise<NormalizedRecord[]>> = {
  baltimore: fetchBaltimore,
  cityprotect: fetchCityprotect,
  cdc_places: fetchCdcPlaces,
  airnow: fetchAirnow,
  nsopw: fetchNsopw,
};

/** Comma-separated source ids to skip when running "all" (e.g. DISABLED_SOURCES=cityprotect,nsopw). */
...
export async function runSource(source: SourceName, jobId: string): Promise<{ added: number; updated: number; removed: number; snapshotFile: string }> {
  ...
  if (freshRecords.length === 0) return { added: 0, updated: 0, removed: 0, snapshotFile: "" };

  const diff = runDiff(source, freshRecords);
  const result = getDb().transaction(() => {
    let added = 0, updated = 0;
    for (const record of diff.added) {
      if (upsertReport({ ...record, id: crypto.randomUUID() }) === "inserted") {
        added++;
    ...
    for (const { previous, current } of diff.updated) {
      if (upsertReport({ ...current, id: crypto.randomUUID() }) === "updated") {
        updated++;
