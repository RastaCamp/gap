...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/" || path === "/api") return json({
      name: "NeighborhoodScore API",
      version: "0.1.0",
      description: "Local safety, crime, CDC PLACES, AirNow, NSOPW",
      ...
      query_params: { q: "Search", source: "baltimore | cityprotect | cdc_places | airnow | nsopw", report_type: "Filter", state: "Filter", page: "Page", per_page: "Per page" },
    });
...
console.log(`NeighborhoodScore API http://localhost:${PORT}  GET /api/reports  POST /api/admin/sync (auth)`);
