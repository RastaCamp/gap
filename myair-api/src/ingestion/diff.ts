...
export function summarizeUpdate(prev: NormalizedRecord, curr: NormalizedRecord): string {
  ...
  const fields: Array<keyof NormalizedRecord> = ["title", "summary", "value", "category", "observed_at"];
  for (const field of fields) {
    if (prev[field] !== curr[field]) changes.push(`${String(field)}: "${prev[field]}" → "${curr[field]}"`);
  }
  return changes.length ? changes.join("; ") : "Content changed (hash mismatch)";
}
...
export function runDiff(source: SourceName, freshRecords: NormalizedRecord[]): RunDiffResult {
  ...
  return { ...diff, snapshotFile, previousRecordCount: previousRecords.length };
}
